...
<div class="main-content">
		<div class="breadcrumbs" id="breadcrumbs">
			...
			<ul class="breadcrumb">
				<li>
					...
					<a href="/wenda/admin.php/Welcome/index.html">后台主页</a>
				</li>
	...
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($i % 2 );++$i;?><tr>
		   <for($i=1;$i<count($list);++$i;>
			<td class="hidden-480"  style="text-align: center;"><?php echo ($t["context"]); ?></td>
			<td style="text-align: center;" id="<?php echo ($t["aid"]); ?>">
				<?php if($t["state"] == 1 ): ?>开启
                <?php else: ?> 关闭<?php endif; ?>
			</td>

			<td style="text-align: center;"><?php echo ($t["createtime"]); ?></td>
			<td>
				<div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
					<a class="green" name="<?php echo ($t["aid"]); ?>"  href="#">
						<i class="icon-pencil bigger-130">修改</i>
						
					</a>
					<a href="javascript:;" class="cz"  onclick="if (confirm('你确定要重置状态吗?')){
						statereast(id='<?php echo ($t["aid"]); ?>',w='<?php echo ($i); ?>')}" >
	<i class="icon-pencil bigger-130"  id="<?php echo ($i); ?>">
							<?php if($t["state"] == 1): ?>置为关闭
                            <?php else: ?>置为开启<?php endif; ?>
						 </i>	
					</a>
                 
					<a class="red" name="<?php echo ($t["aid"]); ?>" href="#" >
						<i class="icon-trash bigger-130">删除</i>
									...
									<tr>
										...
										<td colspan="5"><div class="yahoo2"><?php echo ($Page); ?></div></td>
									</tr>
									</tbody>
...
//alert(000);
	$.ajax({
		url:"/wenda/admin.php/Announce/delannounce",
		data:{'aid':id}, 
		type:"GET",
		dataType:"json",
...
function statereast(id,w){
	...
	var closeing = "置为关闭";
             $.ajax({
				url:"/wenda/admin.php/Announce/updatestate",
				data:{'aid':id}, 
				type:"GET",
				dataType:"json",
				success:function(data){
